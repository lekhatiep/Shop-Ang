...
export class SearchInputComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const subSearchInput =
      this.formSearch.controls.searchText.valueChanges.subscribe((value) => {
  ...
  onSubmit() {
    ...
    if (searchTextControl.valid) {
      this.productService.filterModal.searchText =
        searchTextControl.value ?? '';

      if (this.productService.isInProductDetailPage()) {
        this.router
          .navigate(['/'])
          .then(() => this.productService.loadProductByFilter());
      } else {
    ...
    if (
      searchTextControl.value &&
      !this.historyList.includes(searchTextControl.value)
    ) {
  ...
  onChangeSearchScope() {
    this.searchScopeText =
      this.searchScopeText === 'Trong shop' ? 'Ngoài shop' : 'Trong shop';
  }
